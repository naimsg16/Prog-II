...
public class IntegralTypes extends CommandLineProgram {
    ...
    public void copy(String from, int[] to) {
        ...
        char [] copiar = from.toCharArray();

        for (int i = 0; i < copiar.length; i++) {
                to[i] = copiar[(copiar.length-1)-i];
        }
    }
    ...
    public int[] narrow(int[] from, int toLength) {
        ...
        int[] narrow = new int[toLength];

        for (int i = 0; i < toLength; i++) {
                narrow[i] = from[from.length - toLength + i];
        }
    ...
    public int[] widen(int[] from, int toLength) {
        ...
        int[] widen = new int[toLength];
        int i =0;

        while(i<from.length){
            widen[(toLength-1)-i] = from[(from.length-1)-i];
            i++;
        }

        while(i<toLength){
            widen[(toLength- from.length)-i]=0;
            i++;
        }
